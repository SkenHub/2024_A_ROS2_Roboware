...
gnome-terminal --tab --title="cmd_vel_to_serial_node" -- bash -c "source /opt/ros/humble/setup.bash; source ~/ros2_robocon_ws/2024_A_ROS2_Roboware/install/setup.bash; ros2 run robot_controller cmd_vel_to_serial_node; exec bash"

# Start serial_to_position_node in a new tab
gnome-terminal --tab --title="serial_to_position_node" -- bash -c "source /opt/ros/humble/setup.bash; source ~/ros2_robocon_ws/2024_A_ROS2_Roboware/install/setup.bash; ros2 run robot_controller serial_to_position_node; exec bash"

# Start realsense_position_node in a new tab
gnome-terminal --tab --title="realsense_position_node" -- bash -c "source /opt/ros/humble/setup.bash; source ~/ros2_robocon_ws/2024_A_ROS2_Roboware/install/setup.bash; ros2 run robot_controller realsense_position_node; exec bash"
